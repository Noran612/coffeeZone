...
var createDropzone = (_props = {}) => {
  ...
  const pickerTypes = createMemo(
    () => props.useFsAccessApi ? pickerOptionsFromAccept(props.accept) : void 0
  );
    ...
    if (isEvtWithFiles(event)) {
      Promise.resolve(props.getFilesFromEvent(event)).then((files) => {
        ...
        const isDragAccept = fileCount > 0 && allFilesAccepted({
          files: files.map(
            (fileOrDataTransferItem) => fileOrDataTransferItem instanceof File ? fileOrDataTransferItem : (
              // `file-selector`
  ...
  const openFileDialog = () => {
    if (fsAccessApiWorks) {
      ...
      window.showOpenFilePicker(opts).then(
        (handles) => props.getFilesFromEvent(handles)
      ).then((files) => {
        setFiles(files, null);
        setState("isFileDialogActive", false);
      }).catch((e) => {
